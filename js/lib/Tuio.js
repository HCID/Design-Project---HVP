...
Tuio.Hand = Tuio.Container.extend({
    handId: null,
    fingers: [],
    ...
    initialize: function(params) {
        ...
        this.handId = params.hi;
        this.fingers = params.fing;
    },
    update: function(params) {
        Tuio.Container.prototype.update.call(this, params);
	this.fingers = params.fing;
    },
    updateHand: function(than) {
        Tuio.Container.prototype.updateContainer.call(this, than);
    },
    ...
    fromHand: function(than) {
        return new Tuio.Hand({
            xp: than.getX(),
            yp: than.getY(),
            si: than.getSessionID(),
            sym: than.getHandId(),
            fing: than.getFingers()
        });
    }
});
...
Tuio.Client = Tuio.Model.extend({
    ...
    getTuioHands: function() {
        return _.clone(this.handList);
    },
    ...
    getTuioHand: function(sid) {
        return this.handList[sid];
    },
    ...
    acceptMessage: function(oscMessage) {
        ...
        switch (address) {
            ...
            case "/tuio/2Dcur":
                ...
                break;
            case "/tuio/_hand":
                this.handleHandMessage(command, args);
                break;	
        }
    },

    handleHandMessage: function(command, args) {
    ...
    handSet: function(args) {
        ...
        ySpeed = args[4],
        mAccel = args[5],
	fingers = [args[6], args[7], args[8], args[9], args[10]];

       if (!_.has(this.handList, sid)) {
            var addHand = new Tuio.Hand({
                si: sid,
                hi: -1,
                xp: xPos,
                yp: yPos,
		fing: fingers,
            ...
            this.frameHands.push(addHand);
        } else {
            var than = this.handList[sid];
            if (!than) {
                return;
            }
           if (
                ...
                (than.xSpeed !== xSpeed) ||
                (than.ySpeed !== ySpeed) ||
                (than.motionAccel !== mAccel) ||
                (than.fingers !== fingers)) {

                var updateHand = new Tuio.Hand({
                    si: sid,
                    ci: than.getHandId(),
                    xp: xPos,
                    yp: yPos,
		    ...
		    fing: fingers
                ...
                this.frameHands.push(updateHand);
            }
        }
    },
    handAlive: function(args) {
        var removeHand = null;
        this.newHandList = args;
        this.aliveHandList = _.difference(this.aliveHandList, this.newHandList);

        for (var i = 0, max = this.aliveHandList.length; i < max; i++) {
            removeHand = this.handList[this.aliveHandList[i]];
            if (removeHand) {
                removeHand.remove(this.currentTime);
                this.frameHands.push(removeHand);
            }
        }
    },
    handFseq: function(args) {
	var fseq = args[0],
        lateFrame = false,
        than = null;
        ...
        if (!lateFrame) {
            for (var i = 0, max = this.frameHands.length; i < max; i++) {
                than = this.frameHands[i];
                switch (than.getTuioState()) {
                    case Tuio.Hand.TUIO_REMOVED:
                        this.handRemoved(than);
                        break;
                    case Tuio.Hand.TUIO_ADDED:
                        this.handAdded(than);
                        break;
                    default:
                        this.handDefault(than);
                        break;
                }
            }
            ...
            var buffer = this.aliveHandList;
            this.aliveHandList = this.newHandList;
            this.newHandList = buffer;
        }

        this.frameHands = [];

    },
    handRemoved: function(than) {
        var removeHand = than;
        removeHand.remove(this.currentTime);
        this.trigger("removeTuioHand", removeHand);
        delete this.handList[removeHand.getSessionId()];
    },

    handAdded: function(than) {
        var addHand = new Tuio.Hand({
            ttime: this.currentTime,
            si: than.getSessionId(),
            hi: than.getHandId(),
        ...
        this.handList[addHand.getSessionId()] = addHand;
        this.trigger("addTuioHand", addHand);
    },

    handDefault: function(than) {
        var updateHand = this.handList[than.getSessionId()];
        if (
            (than.getX() !== updateHand.getX() && than.getXSpeed() === 0) ||
            (than.getY() !== updateHand.getY() && than.getYSpeed() === 0)) {
            ...
            updateHand.update({
                ...
                xs: than.getXSpeed(),
                ys: than.getYSpeed(),
                ma: than.getMotionAccel(),
            });
        }
        
        this.trigger("updateTuioHand", updateHand);
    },
